...
function Lections() {
  ...
  useEffect(() => {
    ...
    axios.get("http://95.163.234.208:3500/colors").then(({ data }) => {
      setColors(data);
    });
  }, []);
  ...
  const closeLection = (id) => {
    axios.patch(`http://95.163.234.208:3500/lists/${id}`, { usersId: [] });
    setState(true);
  };
  React.useEffect(() => {
  ...
  const onEditTask = (listId, taskObj) => {
    ...
    setLists(newList);
    axios
      .patch("http://95.163.234.208:3500/tasks/" + taskObj.id, {
        text: newTaskText,
      })
      .catch((e) => {
  ...
  const onCompleteTask = (listId, taskId, completed) => {
    ...
    setLists(newList);
    axios
      .patch("http://95.163.234.208:3500/tasks/" + taskId, {
        completed,
      })
      .catch((e) => {
        alert("Не удалось обновить задачу");
